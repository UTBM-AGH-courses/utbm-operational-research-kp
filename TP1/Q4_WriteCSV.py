...
# ID des actions à mettre dans le sac
ID_actions = np.arange(1, nombre_actions + 1)
prix = np.random.randint(50, 350, size=nombre_actions)
# Gains des actions attendus
benef_attendu = prix*np.random.uniform(0, 0.8, size=nombre_actions)
...
with open("data4.csv", 'w',) as csvfile:
    ...
    for i in range(ID_actions.shape[0]):
        ...
        spamwriter.writerow([f'{ID_actions[i]}'] +
                            [f'{prix[i]}'] + [f'{benef_attendu[i]}'])
    print()
...
def optimize(prix, benef_attendu, population, pop_size, nbr_generations, budget):
    ...
    fitness_derniere_generation = cal_fitness(
        prix, benef_attendu, population, budget)
...
sol_opt, historique_fitness = optimize(
    prix, benef_attendu, population_initiale, pop_size, nbr_generations, budget)
...
print('Avec un portefeuille d\'actions d\'une valeur totale en-devenir de : ',
      np.amax(historique_fitness), '€ par mois, et pour un total dépensé de : ', np.sum(sol_opt * prix), '€')
