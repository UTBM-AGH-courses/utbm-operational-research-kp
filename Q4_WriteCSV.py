...
ID_actions = np.arange(1, nombre_actions + 1)                   # ID des actions à mettre dans le sac
prix = np.random.randint(50, 350, size=nombre_actions)   
benef_attendu = prix*np.random.uniform(0, 0.8, size=nombre_actions) # Gains des actions attendus
...
with open("data4.csv", 'w',) as csvfile:
    ...
    for i in range(ID_actions.shape[0]):
        ...
        spamwriter.writerow([f'{ID_actions[i]}'] + [f'{prix[i]}'] + [f'{benef_attendu[i]}'])
    print()
